...
server_address = '/tmp/timecircuit_ipc_socket'
...
conf_filepath="timecircuit.conf"
...
def get_process_args():
    ...
    if(process_conf["power"] == "on"):
        ...
        process_args[argbrightness]=str(range_map_limit(int(process_conf["brightness"]),0,100,1,16))
    else:
        process_args[argbrightness]=str(0)

    return process_args
  


def start_command(arg=None):
    """
    Start the driver process of the TimeCircuitDisplay
    """
    global process
